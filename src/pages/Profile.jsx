...
export default function Profile({ onBack }) {
  ...
  // ── Visibilité : être trouvable par nom / email ───────────────────────────
  const toggleDiscoverable = async () => {
    const next = !(user.discoverable === true)
    setSavingVisibility(true)
    try {
      await api.updateUser(user.id, { discoverable: next })
      updateUser({ discoverable: next })
      toast.success(next
        ? 'Vous êtes maintenant visible dans les recherches.'
  ...
  const copyShareId = async () => {
    try {
      await navigator.clipboard.writeText(user.shareId)
      setCopied(true)
      setTimeout(() => setCopied(false), 1800)
    } catch {
      toast.error('Copie impossible — sélectionnez l\'identifiant manuellement.')
    }
  }

  // Invalide l'ancien identifiant : les personnes à qui il a été donné ne
  // pourront plus l'utiliser, les groupes déjà rejoints ne bougent pas.
  const regenerateShareId = async () => {
    setRegenerating(true)
    try {
      const shareId = await allocateShareId()
      await api.updateUser(user.id, { shareId })
      updateUser({ shareId })
      setConfirmRegen(false)
      toast.success('Nouvel identifiant généré. L\'ancien ne fonctionne plus.')
    } catch {
      toast.error('Impossible de générer un nouvel identifiant.')
  ...
  return (
    <div style={{ maxWidth: 540, margin: '0 auto' }}>
      ...
      <Section title="Visibilité et partage">
        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 14, lineHeight: 1.5 }}>
          Votre compte est privé : les autres utilisateurs ne peuvent pas vous trouver
          en cherchant votre nom ou votre email. Pour qu'on vous ajoute à un groupe,
          communiquez votre identifiant de partage.
        </div>

        <Field label="Votre identifiant de partage">
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <input
              style={{
                ...inp, flex: 1, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace',
              ...
              aria-label="Votre identifiant de partage"
            />
            <Btn onClick={copyShareId} disabled={!user.shareId}>
              {copied ? '✓ Copié' : 'Copier'}
            </Btn>
          </div>
          <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 6 }}>
            À donner uniquement aux personnes avec qui vous partagez des dépenses.
          </div>
        </Field>

        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, padding: '12px 0', borderTop: '0.5px solid var(--border)' }}>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>
              Être trouvable par nom ou email
            </div>
            <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2, lineHeight: 1.5 }}>
              {user.discoverable === true
                ? 'Activé — n\'importe quel utilisateur peut vous trouver en cherchant votre nom ou votre email, et vous ajouter à un groupe.'
                : 'Désactivé — seul votre identifiant de partage permet de vous ajouter.'}
            </div>
          </div>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, cursor: savingVisibility ? 'wait' : 'pointer' }}>
            <input
              type="checkbox"
              checked={user.discoverable === true}
              onChange={toggleDiscoverable}
              disabled={savingVisibility}
              style={{ width: 18, height: 18, cursor: 'inherit', accentColor: 'var(--green)' }}
            />
            <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
              {user.discoverable === true ? 'Visible' : 'Privé'}
            </span>
          </label>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, paddingTop: 12, borderTop: '0.5px solid var(--border)' }}>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>Régénérer l'identifiant</div>
            <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>
              Si vous l'avez communiqué à la mauvaise personne. Vos groupes actuels ne changent pas.
            </div>
          </div>
          {!confirmRegen ? (
            <Btn onClick={() => setConfirmRegen(true)} disabled={!user.shareId}>Régénérer</Btn>
          ) : (
            <div style={{ display: 'flex', gap: 8 }}>
              <Btn onClick={() => setConfirmRegen(false)}>Annuler</Btn>
              <Btn variant="primary" onClick={regenerateShareId} disabled={regenerating}>
