...
export default function Reminders({ group, groupId, members, expenses, payments, reminders, onRefresh }) {
  ...
  const sendEmailReminder = async (debt) => {
    ...
    try {
      const subject = encodeURIComponent(`Rappel de remboursement — ${group?.name ?? 'NotreTab'}`)
      const body = encodeURIComponent(
        `Bonjour ${debtor?.name ?? ''},\n\n` +
        `${creditor?.name ?? 'Un membre'} te rappelle que tu lui dois ${debt.amount.toFixed(2)} €` +
        ` dans le groupe « ${group?.name ?? 'NotreTab'} ».\n\n` +
        `Merci de régulariser dès que possible !\n\nNotreTab`
      )
      window.open(`mailto:${debtor.email}?subject=${subject}&body=${body}`)
      ...
      toast.success(`Client email ouvert pour ${debtor.name} !`)
      onRefresh()
    } catch {
