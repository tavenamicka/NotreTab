...
export default function ParticipationWizard({ open, onClose, onSaved }) {
  ...
  // Participant search — identifiant de partage, ou nom/email si le compte est visible
  const [searchQuery, setSearchQuery] = useState('')
  const [searchResults, setSearchResults] = useState([])
  const [searchState, setSearchState] = useState('idle')  // idle | loading | ok | empty
  ...
  // Un identifiant de partage trouve n'importe quel compte ; un nom ou un email
  // ne trouve que les comptes ayant activé la visibilité dans leur profil.
  ...
  useEffect(() => {
    ...
    searchTimeout.current = setTimeout(async () => {
      try {
        const canonical = normalizeShareId(q)
        const found = canonical
          ? [await api.findUserByShareId(canonical)].filter(Boolean)
          : await api.searchProfiles(q)
        setSearchResults(found)
        setSearchState(found.length ? 'ok' : 'empty')
      } catch {
        setSearchResults([])
          ...
          <div key="step-2" className={stepDir === 'forward' ? 'step-forward' : 'step-back'}>
            ...
            {!showGuestForm && (
              <div style={{ marginBottom: 8 }}>
                ...
                <input
                  style={inputStyle(false)}
                  placeholder="NT-XXXX-XXXX, nom ou email"
                  value={searchQuery}
                  onChange={e => setSearchQuery(e.target.value)}
                  autoComplete="off"
                />
                <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 5, lineHeight: 1.45 }}>
                  L'identifiant de partage trouve n'importe qui ; le nom ou l'email ne
                  trouvent que les personnes qui se sont rendues visibles.
                </div>
                ...
                {searchState === 'ok' && searchResults.map(u => (
                  <div key={u.id} style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', border: '0.5px solid var(--border)', borderRadius: 'var(--radius)', background: 'var(--bg-secondary)' }}>
                    <Avatar initials={u.initials || initials(u.name)} color={u.color || '#6b6b6b'} textColor={u.textColor || '#fff'} size={28} />
                    <div style={{ flex: 1, fontSize: 13, minWidth: 0 }}>
                      <div style={{ fontWeight: 500 }}>{u.name}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>
                        {u.discoverable ? u.email : u.shareId}
                      </div>
                    </div>
                    {alreadyAdded(u.id)
                      ? <span style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>Déjà ajouté</span>
                      : <button onClick={() => addRegisteredUser(u)} style={{ padding: '5px 12px', background: 'var(--green)', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>Ajouter</button>
                    }
                  </div>
                ))}
                ...
                {searchState === 'empty' && (
                  <div style={{ marginTop: 8, fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                    Aucun compte trouvé. Demandez son identifiant de partage à la personne, ou{' '}
                    <button onClick={() => { setShowGuestForm(true); setGuestEmail(searchQuery.includes('@') ? searchQuery : '') }} style={{ background: 'none', border: 'none', color: 'var(--green-dark)', fontWeight: 500, cursor: 'pointer', fontSize: 12, padding: 0, fontFamily: 'inherit', textDecoration: 'underline' }}>
                      ajoutez-la comme invité
                    </button>.
                  </div>
                )}
              </div>
